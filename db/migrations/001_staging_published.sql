...
-- Ensure one staged record per (dataset_key, run_id) pair.
-- Prevents silent overwrites — the article's FX re-stamp scenario.
CREATE UNIQUE INDEX IF NOT EXISTS uq_staging_records_key_run
    ON staging.records (dataset_key, run_id);
...
-- Enforce one published record per run — a run can only be published once.
CREATE UNIQUE INDEX IF NOT EXISTS uq_published_records_run
    ON published.records (run_id);

-- Used by SnapshotReferenceSource: "the reference for today's run is
-- whatever was last published for this dataset_key". This is what
